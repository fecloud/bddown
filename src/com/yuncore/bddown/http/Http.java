package com.yuncore.bddown.http;
...
import com.yuncore.bddown.http.cookie.Cookie;
import com.yuncore.bddown.http.cookie.CookieContainer;
import com.yuncore.bddown.http.cookie.HttpCookieContainer;
import com.yuncore.bddown.util.Gzip;
import com.yuncore.bddown.util.Log;
import com.yuncore.bddown.util.TextUtil;
...
public class Http {
	...
	public boolean http() throws MalformedURLException, IOException {
		if (DEBUG) {
			...
			Log.d(TAG, String.format("method:%s", method == Method.GET ? "GET" : "POST"));
		}
	...
	private boolean setCookie() {
		...
		if (null != list) {
			if (DEBUG)
				Log.d(TAG, "Set-Cookie list:" + list);
			for (String c : list) {
				if (DEBUG)
					Log.d(TAG, "Set-Cookie one:" + c);
				if (null != c)
	...
	private void printResponeHead() {
		final Map<String, List<String>> headerFields = conn.getHeaderFields();
		Log.d(TAG, "");
		Log.d(TAG, "[===============respone header ===================]");
		for (Entry<String, List<String>> entry : headerFields.entrySet()) {
			if (entry.getKey() != null) {
				Log.d(TAG, String.format("[%s:%s]", entry.getKey(), entry.getValue()));
			} else {
				Log.d(TAG, String.format("[%s]", entry.getValue()));
			}
		}
		Log.d(TAG, "");
	}
